...
//    private String deviceType;
    ...
    private void showConfirmDialog() {
        new AlertDialog.Builder(this).setMessage("添加过程稍稍有点漫长，耐心等一会儿哦~")
    ...
    private void changeStatuss(int Status) {
        switch (Status) {
            case STATUS_WIFI_CONNETCTING:
                tvStatus.setText("请将手机靠近设备，添加过程大约需要等待1分钟，请稍候…");
    ...
    private void addCameraFailed(int errorStep, int errorCode) {
        ...
        switch (errorStep) {
            case ERROR_WIFI_CONNECT:
                ...
                btnLineConnect.setText("有线连接");
                imgAnimation.setImageResource(R.drawable.auto_wifi_failed);
                tvStatus.setText("Wi-Fi连接失败，请重试或返回检查Wi-Fi密码是否输入正确");
                // stopBonjourOnThread();
                break;
            case ERROR_REGIST:
                // stopBonjourOnThread();
                ...
                btnLineConnect.setVisibility(View.GONE);
                imgAnimation.setImageResource(R.drawable.auto_wifi_failed);
                tvStatus.setText("设备未连接到萤石云，请检查设备所在网络是否能正常上网后重试");
                break;
            case ERROR_ADD_CAMERA:
                btnRetry.setVisibility(View.VISIBLE);
                btnLineConnect.setVisibility(View.GONE);
